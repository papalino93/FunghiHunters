...
describe('conflitto: vince la modifica più recente', () => {
  ...
  it('la durata della ricerca e il numero di cercatori viaggiano nella sincronizzazione', async () => {
    const repo = new InMemoryDiaryRepository()
    const original = materialise(draft({ durationMinutes: 45, searchers: 2 }))
    await repo.upsertRaw(at('2026-09-10T07:00:00.000Z', original))
    const backend = new FakeBackend()
    backend.rows.set(original.id, {
      ...original,
      notes: 'aggiornata da un altro telefono',
      durationMinutes: 60,
      searchers: 3,
      updatedAt: '2026-09-10T09:00:00.000Z',
    })
    ...
    expect(stored?.searchers).toBe(3)
  })
})
